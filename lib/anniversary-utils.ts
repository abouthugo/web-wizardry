...
export const ensureTimeIsDefined = (time: FormattedTime): time is Required<FormattedTime> => {
  return (
    ensureDefined(time.days) && ensureDefined(time.hours) && ensureDefined(time.minutes) && ensureDefined(time.seconds)
  )
}
export function getTimeSinceAnniversary(): FormattedTime {
  const durationUnits: DurationUnits = ['days', 'hours', 'minutes', 'seconds', 'millisecond']
