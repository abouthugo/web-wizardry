...
const LogoComponent = ({ width, height, justLogo = false }: LogoPropsI) => {
  return (
    <div className="z-20 flex">
      <Image width={width} height={height} className="h-6 sm:h-9" src="/juicelogo.svg" alt="Juicebox logo" />
...
const HamburgerMenu = ({ onClick, hidden }: withOnClickProps) => (
  <button className="md:hidden z-20 flex w-8 cursor-pointer mr-2" onClick={onClick} onKeyDown={onClick} type="button">
    <div
      className={classNames('grid justify-items-center gap-2 transition-all duration-200 ease-linear', {
        '-rotate-180': !hidden
      })}
    >
      <span
        className={classNames('transition-all ease-in duration-200 h-[2px] w-7 rounded-full bg-white', {
          '-rotate-45': !hidden,
